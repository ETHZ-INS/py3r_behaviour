...
def time_in_state(
    ...
    *,
    ...
    """
    Compute time spent in each state for a boolean or categorical series.

    For boolean series: returns a single-element Series with time where value is True.
    For categorical series: returns a Series with time per unique value.

    Parameters
    ----------
    series : pd.Series
        The input series (boolean or categorical with <= max_unique values).
    fps : float
        Frames per second for time conversion.
    max_unique : int
        Maximum number of unique values allowed for categorical columns.
        Default 50. Series with more unique values will raise ValueError.

    Returns
    -------
    pd.Series
        Index = state values (True for boolean, or unique values for categorical).
        Values = time in seconds spent in each state.

    Raises
    ------
    ValueError
        If the series has more than max_unique unique values.
    ...
    >>> # Boolean series
    >>> s_bool = pd.Series([True, True, False, True, False])
    >>> time_in_state(s_bool, fps=1.0)
    True    3.0
    dtype: float64
    >>> # Categorical series
    ...
    # Boolean column (or boolean-like with True/False)
    if series.dtype == "boolean" or series.dtype == bool or set(unique_vals) <= {True, False}:
        time_true = (series == True).sum() / fps  # noqa: E712
        return pd.Series({True: time_true})

    # Categorical / multi-valued
    if n_unique > max_unique:
        raise ValueError(
            f"Series has {n_unique} unique values (> max_unique={max_unique}). "
            "Only boolean or categorical series with limited unique values are supported."
        )

    results = {}
    for val in sorted(unique_vals, key=lambda x: str(x)):
        results[val] = (series == val).sum() / fps
