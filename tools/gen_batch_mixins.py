...
def _needed_annotation_type_names(methods: list[MethodInfo]) -> set[str]:
    """
    Heuristic: capture likely type symbols from generated parameter strings.
    Keep only capitalized identifiers (e.g. DynamicBoundary, Tracking).
    """
    out: set[str] = set()
    pattern = re.compile(r"\b[A-Za-z_][A-Za-z0-9_]*\b")
    for m in methods:
        for tok in pattern.findall(m.params_src):
            if tok and tok[0].isupper():
                out.add(tok)
    return out


def _extract_top_level_imports_for_names(src: str, names: set[str]) -> list[str]:
    """
    Capture top-level import lines whose imported symbols intersect ``names``.
    This lets generated mixins include annotation symbols that were imported
    in the leaf module outside TYPE_CHECKING.
    """
    if not names:
        return []
    mod = ast.parse(src)
    ...
    for node in mod.body:
        if isinstance(node, ast.ImportFrom):
            selected = [a for a in node.names if (a.asname or a.name) in names]
            if not selected:
                continue
            module = f"{'.' * node.level}{node.module or ''}"
            imported = ", ".join(
                [f"{a.name} as {a.asname}" if a.asname else a.name for a in selected]
            )
...
def _generate_collection_mixin(
    ...
    needed_type_names = _needed_annotation_type_names(methods)
    top_level_type_imports = _extract_top_level_imports_for_names(leaf_src, needed_type_names)
    merged_type_imports = list(dict.fromkeys(type_checking_imports + top_level_type_imports))
    lines = [
        "# AUTO-GENERATED FILE. DO NOT EDIT.",
    ...
    if merged_type_imports:
        lines.append("from typing import TYPE_CHECKING")
        lines.append("if TYPE_CHECKING:")
        lines += [f"    {line}" for line in merged_type_imports]
    lines += [
        "",
        f"class {mixin_class}:",
        "",
